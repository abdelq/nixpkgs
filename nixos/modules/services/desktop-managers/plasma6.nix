...
in
{
  options = {
    ...
    services.desktopManager.plasma6Bigscreen = {
      enable = mkOption {
        type = types.bool;
        default = false;
        description = ''
          Enable the Plasma Bigscreen session, a KDE Plasma interface designed
          for TVs and other large screens operated with a remote control.
        '';
      };
    };
  ...
  config = lib.mkMerge [
    ...
    (mkIf config.services.desktopManager.plasma6Bigscreen.enable {
      services.desktopManager.plasma6.enable = mkDefault true;

      services.displayManager = {
        sessionPackages = [ kdePackages.plasma-bigscreen ];
